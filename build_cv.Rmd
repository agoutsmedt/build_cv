...
```{r position}
readWorkbook(wb, sheet = "Positions") %>% 
  filter(lang == language, cv_type == "short") %>% 
...
if(language == "en"){
  ...
  cat("# Séjours de recherche\n")
}

readWorkbook(wb, sheet = "Positions") %>% 
  filter(lang == language, cv_type == "long") %>% 
...
```{r education}
readWorkbook(wb, sheet = "Education") %>% 
...
publications <- readWorkbook(wb, sheet = "Publications") %>% 
  filter(lang == language) %>% 
  { if(cv == "short") filter(., cv_type == cv) else .}
...
books <- publications %>% 
  filter(category == "BOOK" & type == "published") %>% 
  pull(reference)
...
peer_review <- publications %>% 
  filter(type == "published"  & category != "BOOK") %>% 
  { if(cnrs_rank == TRUE) left_join(., readWorkbook(wb,sheet = "CNRS_ranking"), by = "journal") %>% 
...
```{r teaching}
options(Encoding="UTF-8")
readWorkbook(wb, sheet = "Teaching") %>% 
  filter(lang == language) %>% 
  { if(cv == "short") filter(., cv_type == cv) else . } %>%
...
options(Encoding="UTF-8")
readWorkbook(wb, sheet = "Activities")  %>% 
  filter(lang == language,
...
conferences <- readWorkbook(wb, sheet = "Conferences") %>% 
  filter(lang == language) %>% 
  { if(cv == "short") filter(., cv_type == cv) else .} %>% 
